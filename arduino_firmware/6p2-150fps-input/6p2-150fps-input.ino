...
int checkInputPins(void)
{
    bool state_change = false;

    // Check for changes in pin states
...
void printInputPins(int current_cycle)
{
  Serial.write("@")  // special @ indicates a pin state message to python
  writeLongtoSerial(millis())
  writeLongtoSerial(current_cycle);
  for (int pin_i = 0; pin_i < 4; pin_i++)
  {
      Serial.write(input_state[pin_i]);
  }
  Serial.write("\n");
  Serial.write("\r");
}
...
void writeLongtoSerial(long val)
{
    union u_tag
    {
        byte b[4];
